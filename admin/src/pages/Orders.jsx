      ...
      <div className="order-detail-grid">
        <section className="order-summary-section">
          ...
          <dl className="commerce-detail">
            ...
            <div><dt>Payment</dt><dd className="order-payment-detail"><Badge value={order.payment_status} /><small className={canCollectCod ? "cod-payment-note" : ""}>{canCollectCod ? "Cash on Delivery — mark paid only after collecting cash" : paymentProvider ? label(paymentProvider) : "Payment method unavailable"}</small></dd></div>
            <div><dt>Customer</dt><dd className="order-customer-detail"><span>{order.customer_details?.name || order.customer}</span>{order.customer_details?.email && <small>{order.customer_details.email}</small>}{(order.customer_details?.phone || order.phone) && <small>{order.customer_details?.phone || order.phone}</small>}</dd></div>
