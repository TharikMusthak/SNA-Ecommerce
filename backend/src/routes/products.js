...
export default function Products({ rows = [], onEdit, onDelete, onToggleFeatured }) {
  ...
  return (
    <section>
      ...
      <DataTable label="Products" headers={["Image", "Product", "SKU", "Pack size", "Category", "Price", "Stock", "Rating", "Featured", "Status", "Updated", "Actions"]} emptyMessage={rows.length ? "No products match these filters." : "No products found."} minWidth={1360}>
        {filteredRows.map((product) => (
          <tr key={product.id}>
            ...
            <td><Badge value={Number(product.is_featured) === 1 ? "Featured" : "Not featured"} /></td>
