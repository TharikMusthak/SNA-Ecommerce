...
router.get("/orders/:id/details", manageOrders, async (req, res) => {
  ...
  const [[orderRows], [items], [payments], [statusHistory]] = await Promise.all([
    ...
    pool.query(
      `SELECT oi.*,COALESCE(p.main_image,oi.product_image) AS product_image,p.slug AS product_slug
         FROM order_items oi
...
router.put("/orders/:id/stage", manageOrders, async (req, res) => {
  ...
  try {
    ...
    const msg91Event = { confirmed: "order_confirmed", shipped: "order_shipped", out_for_delivery: "order_out_for_delivery", delivered: "order_delivered", cancelled: "order_cancelled" }[nextStatus];
