...
export async function getRazorpayPayment(paymentId){
  if(!configured())throw Object.assign(new Error("Razorpay is not configured"),{status:503});
  const response=await fetch(`https://api.razorpay.com/v1/payments/${encodeURIComponent(paymentId)}`,{headers:{Authorization:`Basic ${Buffer.from(`${env.razorpay.keyId}:${env.razorpay.keySecret}`).toString("base64")}`}});
  const data=await response.json().catch(()=>({}));
  if(!response.ok)throw Object.assign(new Error("Unable to confirm payment status with Razorpay"),{status:502});
  return data;
}
export async function createRazorpayRefund({ paymentId, amountMinor, receipt }) {
  if (!configured()) throw Object.assign(new Error("Razorpay is not configured"), { status: 503 });
  const response = await fetch(`https://api.razorpay.com/v1/payments/${encodeURIComponent(paymentId)}/refund`, {
    ...
    body: JSON.stringify({ amount: amountMinor, receipt }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw Object.assign(new Error(data.error?.description || "Razorpay could not create the refund"), { status: 502 });
  return data;
}
