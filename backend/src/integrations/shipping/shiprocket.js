...
export function shiprocketConfigured() {
  return Boolean(env.shiprocket.token || (env.shiprocket.email && env.shiprocket.password));
}

export async function shiprocketRequest(path, { method = "GET", body, retry = true } = {}) {
  let response;
  try {
    ...
    throw Object.assign(new Error("Shiprocket is temporarily unavailable"), { status: 502 });
  }
