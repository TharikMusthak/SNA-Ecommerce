...
function Cms({ admin, onLogout }) {
  ...
  const loadDashboard = useCallback(async (days = 30, summaryOnly = false) => {
    ...
    try {
      ...
      const enrichedDashboard = dashboard?.products?.length
        ? {
            ...dashboard,
            products: await enrichProductRatings(dashboard.products),
          }
        : dashboard;
      setData((current) => ({
        ...current,
...
async function enrichProductRatings(products) {
  return Promise.all(
    products.map(async (product) => {
      try {
        const response = await api(
          `/v1/reviews/product/${product.id}?page=1&limit=100`,
        );
        ...
        return {
          ...
          review_count: Number(response.pagination?.total || reviews.length),
        };
      } catch {
        return product;
      }
